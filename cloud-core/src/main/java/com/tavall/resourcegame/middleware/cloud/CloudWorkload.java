...
public record CloudWorkload(
    ...
    public CloudWorkload withDesiredState(WorkloadDesiredState state, Instant now) {
        return new CloudWorkload(workloadId, workloadType, nodeId, name, image, version, ports, environmentVariables,
                secretRefs, volumeMounts, resourceLimits, state, actualState, healthStatus, createdAt, now, metadata);
    }
}
