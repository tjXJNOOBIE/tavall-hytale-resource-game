...
public final class KingdomPlacementCommandSupport implements IResourceGameDomain, IDependencyInjectableConcrete {
    public void handle(CommandContext context, Player player, List<String> tokens) {
        ...
        switch (action) {
            ...
            case "cancel" -> sendResult(context, getPlacementModeService().cancelPlacement(player.getUuid()));
            case "status" -> sendStatus(context, player);
            case "preview" -> sendResult(context, getPlacementModeService().refreshPreview(player));
    ...
    private void sendStatus(CommandContext context, Player player) {
        Optional<PlacementRequest> request = getPlacementModeService().activePlacement(player.getUuid());
        if (request.isEmpty()) {
    ...
    private PlacementResult confirmPlacement(CommandContext context, Player player, List<String> tokens) {
        if (tokens.size() >= 3 && "here".equalsIgnoreCase(tokens.get(2))) {
            ...
            return getPlacementModeService().confirmPlacement(player, currentBlock);
        }
        return getPlacementModeService().confirmPlacementFromAim(player);
    }
    ...
    private PlacementResult handleMove(CommandContext context, Player player, List<String> tokens) {
        ...
        try {
            ...
            return getPlacementModeService().moveStagedPlacement(player, new Vector3i(dx, dy, dz));
