 ...
 */
public final class CustomEntitySpawnService implements ICustomEntitySpawnService, IDependencyInjectableConcrete {
    private static final Logger LOGGER = Logger.getLogger(CustomEntitySpawnService.class.getName());
    ...
    public CustomEntitySpawnService(
            ...
            IFarmsteadMenuService farmsteadMenuService
    ) {
        ...
        this.farmsteadMenuService = Objects.requireNonNull(farmsteadMenuService, "farmsteadMenuService");
    }
    ...
    public void spawn(Player player, String roleToken) {
        ...
        WorldTasks.executeSafe(world, "CustomEntitySpawnService.spawn(" + role.name() + ")", () -> {
            try {
    ...
    public boolean openFromTarget(Player player, Ref<EntityStore> targetRef) {
        ...
        if (role == CustomEntitySpawnRole.FARMSTEAD_STEWARD) {
            boolean opened = farmsteadMenuService.openFarmsteadMenu(player);
            if (opened) {
    ...
    private void openBuildingRole(Player player, PlayerSession session, CustomEntitySpawnRole role) {
        Optional<CastleBuildingData> building = buildingService.resolveBuilding(session.gameState(), role.buildingType().shortKey());
    ...
    private void removeSafely(Ref<EntityStore> ref) {
        ...
        if (ref.getStore().getExternalData() instanceof EntityStore entityStore && entityStore.getWorld() != null) {
            WorldTasks.executeSafe(entityStore.getWorld(), "CustomEntitySpawnService.removeSafely", remove);
            return;
        }
        remove.run();
    }
