...
public final class CompanionWallDefenseService {
    ...
    public List<String> getAvailableWallSlots(UUID ownerPlayerId) {
        List<String> occupied = repository.findWallAssignmentsForPlayer(ownerPlayerId).stream()
    ...
    public double calculateWallCompanionBonus(UUID ownerPlayerId) {
        return repository.findWallAssignmentsForPlayer(ownerPlayerId).stream()
