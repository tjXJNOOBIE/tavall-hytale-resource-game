...
public final class CitizenAggregationCalculationHandler {
    private final CitizenFoodEffectHandler foodEffectHandler;
    private final CitizenMoraleEffectHandler moraleEffectHandler;
    private final CitizenHousingEffectHandler housingEffectHandler;
    private final CitizenClockIntegrationHandler clockIntegrationHandler;
    ...
    public CitizenAggregationCalculationHandler(
        ...
        this.foodEffectHandler = foodEffectHandler;
        this.moraleEffectHandler = moraleEffectHandler;
        this.housingEffectHandler = housingEffectHandler;
        this.clockIntegrationHandler = clockIntegrationHandler;
    }
    ...
    private CitizenProductivitySummary productivitySummary(CitizenSummaryScope scope, List<CitizenData> citizens, Instant now) {
        ...
        double moraleModifier = citizens.isEmpty() ? 1.0 : citizens.stream().mapToDouble(citizen -> moraleEffectHandler.modifier(citizen.moraleState())).average().orElse(1.0);
        double foodModifier = citizens.isEmpty() ? 1.0 : citizens.stream().mapToDouble(citizen -> foodEffectHandler.evaluate(citizen).workEfficiencyModifier()).average().orElse(1.0);
        double housingModifier = citizens.isEmpty() ? 1.0 : citizens.stream().mapToDouble(citizen -> housingEffectHandler.modifier(citizen.housingState())).average().orElse(1.0);
        String kingdomId = citizens.stream().findFirst().map(CitizenData::kingdomId).orElse("kingdom-1");
        double clockPhaseModifier = clockIntegrationHandler.productivityModifier(kingdomId);
