...
public final class ControlCommandParsingHandler {
    public ControlCommand parseConsoleCommand(String input, ControlOperator operator, CommandIssuedFrom issuedFrom, Instant now) {
        ...
        return switch (tokens.getFirst().toLowerCase()) {
            ...
            case "account" -> parseAccountCommand(tokens, operator, issuedFrom, dryRun, now);
            ...
            case "frontend" -> parseFrontendCommand(tokens, operator, issuedFrom, dryRun, now);
    ...
    private ControlCommand parseAccountCommand(ArrayList<String> tokens, ControlOperator operator, CommandIssuedFrom issuedFrom, boolean dryRun, Instant now) {
        requireSize(tokens, 2, "account <status|addxp|setlevel|debug> ...");
        String operation = tokens.get(1).toLowerCase();
        if (operation.equals("status")) {
            requireSize(tokens, 3, "account status <universalPlayerId>");
            return command(ControlCommandType.GET_ACCOUNT_STATUS, operator, issuedFrom, CommandTargetScope.PLAYER, Set.of(), Map.of(
                    "universalPlayerId", tokens.get(2)
            ), dryRun, now);
        }
        if (operation.equals("addxp") || operation.equals("xp")) {
            requireSize(tokens, 4, "account addxp <universalPlayerId> <amount>");
            return command(ControlCommandType.ADD_ACCOUNT_EXPERIENCE, operator, issuedFrom, CommandTargetScope.PLAYER, Set.of(), Map.of(
                    "universalPlayerId", tokens.get(2),
                    "amount", tokens.get(3)
            ), dryRun, now);
        }
        if (operation.equals("setlevel") || operation.equals("level")) {
            requireSize(tokens, 4, "account setlevel <universalPlayerId> <level>");
            return command(ControlCommandType.SET_ACCOUNT_LEVEL, operator, issuedFrom, CommandTargetScope.PLAYER, Set.of(), Map.of(
                    "universalPlayerId", tokens.get(2),
                    "level", tokens.get(3)
            ), dryRun, now);
        }
        if (operation.equals("debug")) {
            requireSize(tokens, 4, "account debug <universalPlayerId> <on|off|status>");
            return command(ControlCommandType.SET_ACCOUNT_DEBUG_MODE, operator, issuedFrom, CommandTargetScope.PLAYER, Set.of(), Map.of(
                    "universalPlayerId", tokens.get(2),
                    "mode", tokens.get(3)
            ), dryRun, now);
        }
        throw new ControlCommandValidationException("Unknown account command: " + operation + ".");
    }
    ...
    private ControlCommand parseFrontendCommand(ArrayList<String> tokens, ControlOperator operator, CommandIssuedFrom issuedFrom, boolean dryRun, Instant now) {
        requireSize(tokens, 4, "frontend kd <category> <rawInput>");
        if (!tokens.get(1).equalsIgnoreCase("kd")) {
            throw new ControlCommandValidationException("Expected frontend kd <category> <rawInput>.");
        }
        return command(ControlCommandType.ROUTE_FRONTEND_KD_COMMAND, operator, issuedFrom, CommandTargetScope.PLATFORM, Set.of(), Map.of(
                "category", tokens.get(2),
                "rawInput", String.join(" ", tokens.subList(3, tokens.size()))
        ), dryRun, now);
    }
