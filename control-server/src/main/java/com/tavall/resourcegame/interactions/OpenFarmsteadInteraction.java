package com.tavall.resourcegame.interactions;
...
public final class OpenFarmsteadInteraction extends SimpleInstantInteraction implements IResourceGameDomain {
    ...
    private boolean openWithCustomEntityService(Player player, Ref<EntityStore> targetRef) {
        try {
            return getCustomEntitySpawnService().openFromTarget(player, targetRef);
    ...
    private boolean openWithWorkerNpcService(Player player, Ref<EntityStore> targetRef) {
        try {
            return getWorkerNpcInteractionService().openFromTarget(player, targetRef);
        } catch (IllegalStateException ex) {
    ...
    private boolean openWithBuildingService(Player player, Ref<EntityStore> targetRef) {
        try {
            return getBuildingInteractionService().openFromTarget(player, targetRef);
        } catch (IllegalStateException ex) {
