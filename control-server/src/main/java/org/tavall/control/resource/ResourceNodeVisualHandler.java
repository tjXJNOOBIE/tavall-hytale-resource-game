 ...
 */
public final class ResourceNodeVisualHandler implements IResourceNodeVisualHandler, IDependencyInjectableConcrete {
    private static final Logger LOGGER = Logger.getLogger(ResourceNodeVisualHandler.class.getName());
    private static final int NODE_PLACEMENT_BLOCK_RADIUS = 10;

    private final IResourceNodeHandler resourceNodeHandler;
    private final ResourceNodeStructureHandler structureHandler;
    ...
    public ResourceNodeVisualHandler(
            IResourceNodeHandler resourceNodeHandler,
            ResourceNodeStructureHandler structureHandler,
            WorldLabelHandler worldLabelHandler,
            StructureProtectionHandler protectionHandler
    ) {
        this.resourceNodeHandler = resourceNodeHandler;
    ...
    public void clearNodes(UUID playerId) {
        ...
        for (Map.Entry<UUID, ResourceNodeVisualRefs> entry : refsByNode.entrySet()) {
            ...
            if (world != null) {
                WorldTasks.executeSafe(world, "ResourceNodeVisualHandler.clearRefsOnWorld", () -> clearRefsOnWorld(world, refs));
                continue;
            }
            removeRefs(refs);
        }
    ...
    private void rebuildNodes(UUID playerId, PlayerGameState state) {
        ...
        for (ResourceNodeData node : nodes) {
            ...
            WorldTasks.executeSafe(world, "ResourceNodeVisualHandler.rebuildNode(" + node.nodeId() + ")", () -> {
                ...
                protectionHandler.replaceStructure(
                        ...
                        structureHandler.ensureNodeSite(world, node, summary)
                );
    ...
    private void clearRefsOnWorld(World world, ResourceNodeVisualRefs refs) {
        structureHandler.clearNodeSite(world, refs.worldPosition());
        removeRefs(refs);
    }
