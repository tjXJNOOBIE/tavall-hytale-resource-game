 ...
 */
public final class PlayerTeleportHandler implements IPlayerTeleportHandler, IDependencyInjectableConcrete {
    ...
    public void teleportAfterDelay(Player player, Vector3d position, long delayMillis) {
        HytaleServer.SCHEDULED_EXECUTOR.schedule(
                () -> {
                    ...
                    WorldTasks.executeSafe(world, "PlayerTeleportHandler.teleportAfterDelay", () -> teleport(player, position));
                },
                delayMillis,
    ...
    public void teleport(Player player, World targetWorld, Vector3d position) {
        ...
        if (store.getExternalData() instanceof EntityStore entityStore) {
            ...
            if (currentWorld != null) {
                WorldTasks.executeSafe(currentWorld, "PlayerTeleportHandler.teleport", applyTeleport);
                return;
            }
        }
        applyTeleport.run();
    ...
    public void moveWithoutTeleportAck(Player player, Vector3d position) {
        ...
        if (store.getExternalData() instanceof EntityStore entityStore) {
            World currentWorld = entityStore.getWorld();
            if (currentWorld != null) {
                WorldTasks.executeSafe(currentWorld, "PlayerTeleportHandler.moveWithoutTeleportAck", applyMove);
                return;
            }
        }
        applyMove.run();
    ...
    public void orientPlayer(Player player, Vector3d lookTarget) {
        ...
        if (store.getExternalData() instanceof EntityStore entityStore) {
            World currentWorld = entityStore.getWorld();
            if (currentWorld != null) {
                WorldTasks.executeSafe(currentWorld, "PlayerTeleportHandler.orientPlayer", applyOrientation);
                return;
            }
        }
