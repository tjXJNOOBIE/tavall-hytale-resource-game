 ...
 */
public final class PopulationDisplayHandler implements PopulationDisplayGateway {
    private static final Logger LOGGER = Logger.getLogger(PopulationDisplayHandler.class.getName());
    ...
    public PopulationDisplayHandler(PopulationDisplayConfig displayConfig, WorldLabelHandler worldLabelHandler) {
    ...
    private void removeSafely(Ref<EntityStore> ref) {
        ...
        if (store.getExternalData() instanceof EntityStore entityStore) {
            ...
            if (world != null) {
                WorldTasks.executeSafe(world, "PopulationDisplayHandler.removeSafely", remove);
                return;
            }
        }
        remove.run();
    ...
    private Ref<EntityStore> spawnAnchor(World world, Vector3d position, String label) {
        if (world == null || position == null || label == null || label.isBlank() || worldLabelHandler == null) {
            return null;
        }
        Vector3d labelPosition = new Vector3d(position.getX(), position.getY() + 1.15D, position.getZ());
        return worldLabelHandler.spawnLabel(world, labelPosition, label);
    }
