...
/**
 * Applies Resource Game production chrome to UI documents before HyUI parses them.
 */
public final class UiPageMarkupDecorator {
    ...
    private UiPageMarkupDecorator() {
    }

    public static String load(String resourcePath) {
        String normalizedPath = normalizeResourcePath(resourcePath);
        ClassLoader classLoader = UiPageMarkupDecorator.class.getClassLoader();
        try (InputStream stream = classLoader.getResourceAsStream(normalizedPath)) {
            if (stream == null) {
                throw new IllegalArgumentException("UI resource was not found: " + normalizedPath);
            }
            return new String(stream.readAllBytes(), StandardCharsets.UTF_8);
        } catch (IOException exception) {
            throw new IllegalStateException("Unable to read UI resource: " + normalizedPath, exception);
        }
    }
