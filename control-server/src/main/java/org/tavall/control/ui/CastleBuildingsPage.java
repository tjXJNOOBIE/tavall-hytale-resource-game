...
public final class CastleBuildingsPage extends BaseUiPage {
    ...
    private static Map<String, ?> templateData(Player player, UiNavigationContext context, PlayerGameState state, ICastleBuildingHandler buildingHandler) {
        return Map.ofEntries(
                Map.entry("FarmsteadStatus", status(player, state, buildingHandler, BuildingType.FARMSTEAD)),
                Map.entry("LumberMillStatus", status(player, state, buildingHandler, BuildingType.LUMBER_MILL)),
                Map.entry("IronWorksStatus", status(player, state, buildingHandler, BuildingType.IRON_WORKS)),
                Map.entry("BarracksStatus", status(player, state, buildingHandler, BuildingType.BARRACKS)),
                Map.entry("WorkshopStatus", status(player, state, buildingHandler, BuildingType.WORKSHOP)),
                Map.entry(
                        "FooterStatus",
    ...
    private static List<UiActionBinding> bindings() {
        return List.of(
                UiActionBinding.action("#StageFarmsteadButton", UiActions.BUILDING_STAGE, BuildingType.FARMSTEAD.shortKey(), UiPageType.CASTLE_BUILDINGS),
                UiActionBinding.action("#StageLumberMillButton", UiActions.BUILDING_STAGE, BuildingType.LUMBER_MILL.shortKey(), UiPageType.CASTLE_BUILDINGS),
                UiActionBinding.action("#StageIronWorksButton", UiActions.BUILDING_STAGE, BuildingType.IRON_WORKS.shortKey(), UiPageType.CASTLE_BUILDINGS),
                UiActionBinding.action("#StageBarracksButton", UiActions.BUILDING_STAGE, BuildingType.BARRACKS.shortKey(), UiPageType.CASTLE_BUILDINGS),
                UiActionBinding.action("#StageWorkshopButton", UiActions.BUILDING_STAGE, BuildingType.WORKSHOP.shortKey(), UiPageType.CASTLE_BUILDINGS),
                UiActionBinding.action("#BackButton", UiActions.OPEN_CASTLE_MAIN)
        );
    }

    private static String status(Player player, PlayerGameState state, ICastleBuildingHandler buildingHandler, BuildingType buildingType) {
        Optional<CastleBuildingData> building = buildingHandler.resolveBuilding(state, buildingType.shortKey());
        if (building.isEmpty()) {
            return "Missing | Place in " + buildingType.areaType().displayName() + " | " + buildingType.description();
        }
        CastleBuildingSummary summary = buildingHandler.summary(player.getUuid(), state, building.get(), Instant.now());
