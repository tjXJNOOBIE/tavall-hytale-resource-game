...
public final class VisualVerificationControlHandler implements IDependencyInjectableConcrete, IVisualVerificationControlHandler {
    ...
    private void handleRequest(String requestId, Properties request) {
        ...
        WorldTasks.executeSafe(
                world,
                "VisualVerificationControlHandler.handleRequestOnWorldThread",
                () -> {
                    if (closeRequest) {
    ...
    private void handleRequestOnWorldThread(String requestId, UiScreenKey pageType, PlayerRef playerRef, World expectedWorld) {
        ...
        if (currentWorld != expectedWorld) {
            WorldTasks.executeSafe(
                    currentWorld,
                    "VisualVerificationControlHandler.handleRequestOnCurrentWorld",
        ...
        playerDataHandler.ensureSession(player).whenComplete((ignored, throwable) -> {
            if (throwable != null) {
    ...
    private void openUiOnPlayerWorld(String requestId, UiScreenKey pageType, PlayerRef playerRef) {
        ...
        WorldTasks.executeSafe(world, "VisualVerificationControlHandler.openUi", () -> {
            Player player = resolvePlayerOnWorldThread(playerRef);
            if (player == null) {
                writeAck(requestId, "error", "Live player component is not ready.", null);
                return;
    ...
    private void closeUiOnWorldThread(String requestId, PlayerRef playerRef, World expectedWorld) {
        ...
        if (currentWorld != expectedWorld) {
            WorldTasks.executeSafe(
                    currentWorld,
                    "VisualVerificationControlHandler.closeUiOnCurrentWorld",
