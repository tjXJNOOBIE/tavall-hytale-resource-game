...
public final class KingdomNodeCommandSupport implements IDependencyInjectableConcrete {
    ...
    public KingdomNodeCommandSupport(
        ...
        this.promptLaneHandler = Objects.requireNonNull(promptLaneHandler, "promptLaneHandler");
    ...
    private void handleNodePlacement(CommandContext context, Player player, List<String> tokens) {
        ...
        placementModeHandler.armNodePlacement(player, resourceType);
    ...
    private void handleNodeList(CommandContext context, PlayerGameState state) {
        List<ResourceNodeData> nodes = resourceNodeHandler.listNodes(state);
        if (nodes.isEmpty()) {
            context.sendMessage(Message.raw("No placed nodes. Use /kd place node <type>.").color("yellow"));
            return;
        }
        for (int index = 0; index < nodes.size(); index++) {
            context.sendMessage(Message.raw(resourceNodeHandler.summaryLine(state, nodes.get(index), index + 1)).color("yellow"));
        }
    }
    ...
    private void handleNodeAlign(CommandContext context, Player player, List<String> tokens, PlayerSession session) {
        ...
        promptLaneHandler.alignPlayer(player, node.get());
        focusedWorldOverrideHandler.markNode(session.playerId(), node.get().nodeId());
    ...
    private void handleNodeAssign(CommandContext context, List<String> tokens, PlayerSession session, boolean deltaMode) {
        ...
        Optional<ResourceNodeData> node = resourceNodeHandler.resolveNode(session.gameState(), tokens.get(2));
        if (node.isEmpty()) {
        ...
        PlayerGameState updatedState = deltaMode
                ? resourceNodeHandler.addTroops(session.playerId(), node.get().nodeId(), amount.getAsInt(), Instant.now())
                : resourceNodeHandler.assignTroops(session.playerId(), node.get().nodeId(), amount.getAsInt(), Instant.now());
    ...
    private void handleNodeRecall(CommandContext context, List<String> tokens, PlayerSession session) {
        ...
        if (tokens.size() >= 4 && "all".equalsIgnoreCase(tokens.get(3))) {
            updatedState = resourceNodeHandler.assignTroops(session.playerId(), node.get().nodeId(), 0, Instant.now());
            ...
            updatedState = resourceNodeHandler.addTroops(session.playerId(), node.get().nodeId(), -amount.getAsInt(), Instant.now());
        } else {
            updatedState = resourceNodeHandler.addTroops(session.playerId(), node.get().nodeId(), -1, Instant.now());
        }
        resourceNodeVisualHandler.refreshNodes(session.playerId(), updatedState);
        uiNavigator.refreshTrackedPage(session.playerId(), updatedState);
        sendNodeSummary(context, updatedState, node.get().nodeId());
    ...
    private void handleNodeStock(CommandContext context, List<String> tokens, PlayerSession session) {
        ...
        PlayerGameState updatedState = resourceNodeHandler.setStock(session.playerId(), node.get().nodeId(), amount.getAsInt(), Instant.now());
        resourceNodeVisualHandler.refreshNodes(session.playerId(), updatedState);
        uiNavigator.refreshTrackedPage(session.playerId(), updatedState);
        sendNodeSummary(context, updatedState, node.get().nodeId());
    }
    ...
    private void handleNodePillage(CommandContext context, Player player, List<String> tokens, PlayerSession session) {
        ...
        ResourceNodePillageResult pillageResult = resourceNodeHandler.pillageNode(session.playerId(), node.get().nodeId(), Instant.now());
    ...
    private void handleNodeGoto(CommandContext context, Player player, List<String> tokens, PlayerSession session) {
        ...
        playerTeleportHandler.teleport(player, playerTeleportHandler.standingPosition(player, node.get().location().standingBaseVector()));
    ...
    private void handleNodeRemove(CommandContext context, List<String> tokens, PlayerSession session) {
        ...
        PlayerGameState updatedState = resourceNodeHandler.removeNode(session.playerId(), node.get().nodeId(), Instant.now());
        resourceNodeVisualHandler.refreshNodes(session.playerId(), updatedState);
        uiNavigator.refreshTrackedPage(session.playerId(), updatedState);
    ...
    private void handleNodeClear(CommandContext context, PlayerSession session) {
        PlayerGameState updatedState = resourceNodeHandler.clearNodes(session.playerId(), Instant.now());
        resourceNodeVisualHandler.refreshNodes(session.playerId(), updatedState);
        uiNavigator.refreshTrackedPage(session.playerId(), updatedState);
    ...
    private void sendNodeSummary(CommandContext context, PlayerGameState state, UUID nodeId) {
        Optional<ResourceNodeData> node = resourceNodeHandler.findNode(state, nodeId);
        if (node.isEmpty()) {
            context.sendMessage(Message.raw("Node updated.").color("green"));
            return;
        }
        int index = resourceNodeHandler.listNodes(state).indexOf(node.get()) + 1;
        context.sendMessage(Message.raw(resourceNodeHandler.summaryLine(state, node.get(), index)).color("green"));
    }
    ...
    private Optional<ResourceNodeData> resolveNode(CommandContext context, Player player, PlayerSession session, String token) {
        if ("focus".equalsIgnoreCase(token)) {
            Optional<UUID> focusedNodeId = focusedWorldInteractionHandler.focusedNodeId(player);
            if (focusedNodeId.isEmpty()) {
                context.sendMessage(Message.raw("No focused node in front of you.").color("red"));
                return Optional.empty();
            }
            return resourceNodeHandler.findNode(session.gameState(), focusedNodeId.get());
        }
        Optional<ResourceNodeData> node = resourceNodeHandler.resolveNode(session.gameState(), token);
        if (node.isEmpty()) {
            context.sendMessage(Message.raw("Node not found.").color("red"));
        }
        return node;
