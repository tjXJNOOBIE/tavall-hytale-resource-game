...
public final class HytaleHttpControlCommandClient implements FrontendControlCommandClient {
    private final URI ingressUri;
    private final HttpClient httpClient;
    private final ObjectMapper objectMapper;

    public HytaleHttpControlCommandClient(URI ingressUri) {
        this(
                ingressUri,
    ...
    HytaleHttpControlCommandClient(URI ingressUri, HttpClient httpClient, ObjectMapper objectMapper) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                return FrontendCommandVerificationResult.rejected(
                        envelope,
                        "Control ingress rejected Hytale command with HTTP status " + response.statusCode() + "."
                );
            }
            return objectMapper.readValue(response.body(), FrontendCommandVerificationResult.class);
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            return FrontendCommandVerificationResult.rejected(envelope, "Hytale command submission was interrupted.");
        } catch (IOException exception) {
            // Hytale command handlers need a normal verification result, not a thrown transport exception.
            return FrontendCommandVerificationResult.rejected(envelope, "Hytale command submission failed: " + exception.getMessage());
        }
    }
}
