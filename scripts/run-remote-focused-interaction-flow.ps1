param(
    ...
    [string]$ScenarioScriptPath = "F:/workspace/TavallMonoRepo/tavall-java-hytale-games/tavall-hytale-resource-game/scripts/remote-focused-interaction-flow.mjs",
    [string]$PluginJarPath = "F:/workspace/TavallMonoRepo/tavall-java-hytale-games/tavall-hytale-resource-game/core/target/tavall-hytale-resource-game.jar",
    ...
    [int]$Port = 5522,
    [string]$Username = "FocusBot",
    [string]$StableUuid = "823e4567-e89b-12d3-a456-426614174000",
    [string]$LogDir = ""
)
...
$baseName = "remote-focused-interaction-flow-{0}" -f $timestamp
...
Write-LogLine ("[{0}] Starting remote focused interaction flow" -f $startedAt)
...
if ($exitCode -ne 0) {
    throw "Remote focused interaction scenario failed"
}
...
Write-LogLine ("[{0}] Focused interaction flow passed" -f (Get-Date).ToString("o"))
