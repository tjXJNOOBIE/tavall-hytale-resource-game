param(
    ...
    [string]$CloudControlPlaneJarPath = "F:/workspace/TavallMonoRepo/tavall-java-hytale-games/tavall-hytale-resource-game/distribution/control-server/application.jar",
...
$cloudControlPlaneLibDirectory = Join-Path (Split-Path -Parent $CloudControlPlaneJarPath) "libs"
if (-not (Test-Path $cloudControlPlaneLibDirectory -PathType Container)) {
    throw "Cloud control plane dependency directory not found at $cloudControlPlaneLibDirectory"
}
...
exec java --enable-preview -cp '/srv/control-plane/cloud-control-plane.jar:/srv/control-plane/libs/*' org.tavall.control.cli.ControlConsoleApplication >> /srv/control-plane/logs/cloud-control-plane.out.log 2>&1
'@
...
Invoke-Checked -FilePath "ssh.exe" -Arguments @("-F", $SshConfigPath, $SshAlias, "mkdir -p '$RemoteCloudControlPlaneDir' && rm -rf '$RemoteCloudControlPlaneDir/libs.new'") -FailureMessage "Failed to prepare remote cloud control plane directory."
...
Invoke-Checked -FilePath "scp.exe" -Arguments @("-F", $SshConfigPath, "-r", $cloudControlPlaneLibDirectory, "$SshAlias`:$RemoteCloudControlPlaneDir/libs.new") -FailureMessage "Failed to copy cloud control plane dependencies."
