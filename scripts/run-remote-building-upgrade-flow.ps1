param(
    ...
    [string]$ScenarioScriptPath = "F:/workspace/TavallMonoRepo/tavall-java-hytale-games/tavall-hytale-resource-game/scripts/remote-building-upgrade-flow.mjs",
    ...
    [int]$Port = 5522,
    [string]$Username = "BuildingBot",
    [string]$StableUuid = "a23e4567-e89b-12d3-a456-426614174000",
    [string]$LogDir = ""
)
...
$baseName = "remote-building-upgrade-flow-{0}" -f $timestamp
...
Write-LogLine ("[{0}] Starting remote building upgrade flow" -f $startedAt)
...
Write-LogLine ("[{0}] Building upgrade flow passed" -f (Get-Date).ToString("o"))
