...
public final class MinecraftHttpControlCommandClient implements FrontendControlCommandClient {
    private final URI ingressUri;
    private final HttpClient httpClient;
    private final ObjectMapper objectMapper;

    public MinecraftHttpControlCommandClient(URI ingressUri) {
        this(
                ingressUri,
    ...
    MinecraftHttpControlCommandClient(URI ingressUri, HttpClient httpClient, ObjectMapper objectMapper) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                return FrontendCommandVerificationResult.rejected(
                        envelope,
                        "Control ingress rejected Minecraft command with HTTP status " + response.statusCode() + "."
                );
            }
            return objectMapper.readValue(response.body(), FrontendCommandVerificationResult.class);
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            return FrontendCommandVerificationResult.rejected(envelope, "Minecraft command submission was interrupted.");
        } catch (IOException exception) {
            return FrontendCommandVerificationResult.rejected(envelope, "Minecraft command submission failed: " + exception.getMessage());
        }
    }
}
